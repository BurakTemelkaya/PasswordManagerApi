...
public class PasswordManager : IPasswordService
{
    ...
    public async Task<ICollection<Password>> AddRangeAsync(ICollection<Password> passwords)
    {
        ICollection<Password> addedPasswords = await _passwordRepository.AddRangeAsync(passwords);
    ...
    public async Task<IPaginate<Password>?> GetListByDynamicAsync(Core.Persistence.Dynamic.DynamicQuery dynamicQuery, Expression<Func<Password, bool>>? predicate = null, Func<IQueryable<Password>,
        IIncludableQueryable<Password, object>>? include = null, int index = 0, int size = 10,
        bool withDeleted = false, bool enableTracking = true, CancellationToken cancellationToken = default)
    {
        IPaginate<Password> passwordList = await _passwordRepository.GetListByDynamicAsync(
            dynamicQuery,
            predicate,
            include,
            index,
